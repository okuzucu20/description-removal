...
def clipaway_projection_block(bg_embeds, fg_embeds):
    # Compute the dot product between bg_embeds and fg_embeds along the feature dimension
    projected_vector_magnitude = (bg_embeds * fg_embeds).sum(dim=1, keepdim=True) / fg_embeds.norm(dim=1, keepdim=True)
    
    # Calculate the projected vector
    projected_vector = projected_vector_magnitude * fg_embeds / fg_embeds.norm(dim=1, keepdim=True)
    
    # Subtract the projected vector from bg_embeds
    return bg_embeds - projected_vector

@torch.no_grad()  
def process_batch_val(batch, config, mlp, alpha_clip, alpha_clip_preprocess, mask_transform, projection_block, ip_adapter, ip_adapter_inpaint, epoch, iteration_count):
    ...
    with torch.no_grad():
        ...
        mlp_output = mlp(torch.cat([alpha_clip_fg_focused_embeddings, alpha_clip_bg_focused_embeddings], dim=-1))
        original_clipaway = clipaway_projection_block(alpha_clip_bg_focused_embeddings, alpha_clip_fg_focused_embeddings)
...
@torch.no_grad()
def process_val_dataloader(val_dataloader, config, mlp, alpha_clip, alpha_clip_preprocess, mask_transform, projection_block, ip_adapter, ip_adapter_inpaint, epoch, iteration_count):
    ...
    for batch in val_dataloader:
        processed_batch = process_batch_val(batch, config, mlp, alpha_clip, alpha_clip_preprocess, mask_transform, projection_block, ip_adapter, ip_adapter_inpaint, epoch, iteration_count)
        if iteration_count == 0:
    ...
    return images_, masks_, fg_focused, bg_focused, fg_focused_inpaint, bg_focused_inpaint, original_clipaway, original_clipaway_inpaint, mlp_output, mlp_output_inpaint

@torch.no_grad()
def validate(config, mlp, val_dataloader, ip_adapter, ip_adapter_inpaint, alpha_clip, alpha_clip_preprocess, mask_transform, projection_block, epoch, logger, iteration_count):
    # Perform validation
    mlp.eval()
    images_, masks_, fg_focused, bg_focused, fg_focused_inpaint, bg_focused_inpaint, original_clipaway, original_clipaway_inpaint, mlp_output, mlp_output_inpaint = process_val_dataloader(val_dataloader, config, mlp, alpha_clip, alpha_clip_preprocess, mask_transform, projection_block, ip_adapter, ip_adapter_inpaint, epoch, iteration_count)
...
def training_step(config, mlp, optimizer, batch, loss, alpha_clip, alpha_clip_preprocess, mask_transform, clip_model, clip_transform):
    if config["dtype"] == "float16":
        dtype = torch.float16
    else:
        dtype = torch.float32
    ...
    preprocessed_bg_image = clip_transform(bg_image) #TODO check if this is correct
    combined_image = batch['combined_image'].to(config['device'], dtype=dtype)
    mask = batch['mask'].to(config['device'], dtype=dtype)
    fg_word1 = batch['fg_word1']
    object_count_is_2 = batch['object_count_is_2']
    
    # get fg_word2 if object_count_is_2 is True use batchprocessing
    fg_word2 = [batch['fg_word2'][i] if object_count_is_2[i] else "" for i in range(len(object_count_is_2))]
            
    with torch.no_grad():
        alpha_clip_fg_focused_embeddings = get_alpha_clip_embedding(combined_image, mask, alpha_clip, alpha_clip_preprocess, mask_transform, config['device'], dtype)
        alpha_clip_bg_focused_embeddings = get_alpha_clip_embedding(combined_image, get_complement_of_mask(mask), alpha_clip, alpha_clip_preprocess, mask_transform, config['device'], dtype)        
        fg1 = get_alpha_clip_text_embedding(fg_word1, alpha_clip, config['device'], dtype)
        fg2 = get_alpha_clip_text_embedding(fg_word2, alpha_clip, config['device'], dtype)        
        bg_only_image_embeds = clip_model.encode_image(preprocessed_bg_image)
        
        
                
    optimizer.zero_grad()
    mlp_input = torch.cat([alpha_clip_fg_focused_embeddings, alpha_clip_bg_focused_embeddings], dim=-1)
    mlp_output = mlp(mlp_input)

    loss_f, loss_r, angle, angle_after_restriction = loss(mlp_output, fg1, fg2,object_count_is_2,  bg_only_image_embeds)

    return loss_f, loss_r, angle, angle_after_restriction
...
def initialize_training_setup(config):
    ...
    mlp = MLPNetwork(2 * 768, 768, 768, 6, do_zero_initialize=config['do_zero_initialize']).to(config['device'])
    ...
    accelerator = Accelerator(kwargs_handlers=[DistributedDataParallelKwargs(find_unused_parameters=True)], gradient_accumulation_steps=config['gradient_accumulation_steps'])
    optimizer = torch.optim.AdamW(mlp.parameters(), lr=config['lr'], weight_decay=config['weight_decay'])
    return mlp, optimizer, accelerator, alpha_clip, alpha_clip_preprocess, mask_transform, ip_adapter, ip_adapter_inpaint, projection_block, clip_model 

def train(config: OmegaConf, logger: SummaryWriter):
    clip_transform = transforms.Normalize((0.48145466, 0.4578275, 0.40821073), (0.26862954, 0.26130258, 0.27577711)) #TODO check if this is correct
    mlp, optimizer, accelerator, alpha_clip, alpha_clip_preprocess, mask_transform, ip_adapter, ip_adapter_inpaint, projection_block, clip_model = initialize_training_setup(config)
    ...
    forgetting_loss = ForgettingLoss()
    resctricted_gradient_loss = RestrictedGradientLoss(mlp, forgetting_loss, reconstruction_loss, f_coeff=config['forget_lambda'])
    

    mlp, optimizer, training_dataloader, val_dataloader = accelerator.prepare(mlp, optimizer, training_dataloader, val_dataloader)
    iteration_count = 0
    for epoch in tqdm(range(config['num_epochs']), desc="Epochs"):
        reconstruct_loss_epoch = 0
        forgetting_loss_epoch = 0
        total_loss_epoch = 0
        
        for batch in training_dataloader:
            with accelerator.accumulate(mlp):
                forget_loss, reconstruct_loss, angle_before_restrict, angle_after_restrict = training_step(config, mlp, optimizer, batch, 
                                                                                                        resctricted_gradient_loss, alpha_clip, 
                                                                                                        alpha_clip_preprocess, mask_transform, clip_model, clip_transform)
                
                total_loss = forget_loss + reconstruct_loss
                
                
                # reconstruct_loss = accelerator.gather(reconstruct_loss).mean().item()
                # forget_loss = accelerator.gather(forget_loss).mean().item()
                # total_loss = reconstruct_loss + forget_loss
                
                if accelerator.is_main_process:
                    reconstruct_loss_epoch += reconstruct_loss
                    forgetting_loss_epoch += forget_loss
                    total_loss_epoch += total_loss
                    
                print(f"Iteration: {iteration_count}, Reconstruction Loss: {reconstruct_loss}, Forgetting Loss: {forget_loss}, Total Loss: {total_loss}, Angle Before Restrict: {angle_before_restrict}, Angle After Restrict: {angle_after_restrict}")
            ...
            if accelerator.is_main_process:
                logger.add_scalar("reconstruction_loss", reconstruct_loss, iteration_count)
                logger.add_scalar("forgetting_loss", forget_loss, iteration_count)
                logger.add_scalar("total_loss", total_loss, iteration_count)
                logger.add_scalars("angles", {
                    "angle_before_restrict": angle_before_restrict,
                    "angle_after_restrict": angle_after_restrict
                }, iteration_count)

            if iteration_count % config['eval_interval'] == 0:
                accelerator.wait_for_everyone()
                if accelerator.is_main_process:
                    validate(config, mlp, val_dataloader, ip_adapter, ip_adapter_inpaint, alpha_clip, alpha_clip_preprocess, 
                            mask_transform, projection_block, epoch, logger, iteration_count)
                    model_to_save = accelerator.unwrap_model(mlp)
                    torch.save(model_to_save.state_dict(), f"{config['output_dir']}/mlp.pth")
            iteration_count += 1
                    

        accelerator.wait_for_everyone()
        if accelerator.is_main_process:
            logger.add_scalar("reconstruction_loss_epoch", reconstruct_loss_epoch, epoch)
            logger.add_scalar("forgetting_loss_epoch", forgetting_loss_epoch, epoch)

if __name__ == "__main__":
    args = parse_args()
    config = OmegaConf.load(args.config)
    post_fix = config['forget_lambda']
    logger = SummaryWriter(config['output_dir']+f"/xx)
    train(config, logger)
